...
const footerLabels: Record<Locale, { home: string; services: string; areas: string; contact: string; privacy: string; terms: string; request: string; rights: string; description: string; serviceNote: string; disclaimer: string; serviceTypes: string; customerInfo: string; defaultServiceItems: string[] }> = {
  ...
  ru: { home: 'Главная', services: 'Услуги', areas: 'Районы', contact: 'Контакты', privacy: 'Политика приватности', terms: 'Условия сервиса', request: 'Заявка', rights: 'Все права защищены.', description: 'Planetlocksmiths помогает с мобильными автомобильными locksmith-заявками: открытие авто, замена ключей, программирование брелков, transponder-ключи, сломанные ключи и зажигание.', serviceNote: 'Доступность, скорость выезда и финальная цена зависят от локации, автомобиля, типа ключа, деталей, времени и сложности работы.', disclaimer: 'Отправка заявки не гарантирует мгновенную доступность. Детали услуги должны подтверждаться до начала работы.', serviceTypes: 'Авто-услуги', customerInfo: 'Информация клиенту', defaultServiceItems: ['Открытие авто', 'Замена ключей', 'Программирование брелков', 'Transponder-ключи', 'Помощь с зажиганием'] },
}
...
export default async function Footer({ locale }: FooterProps) {
  ...
  return (
    <footer className="relative overflow-hidden border-t border-white/10 bg-bg pb-24 pt-14 text-muted md:pb-12">
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_20%_0%,rgba(77,162,255,0.12),transparent_28rem),radial-gradient(circle_at_84%_18%,rgba(214,168,95,0.08),transparent_24rem)]" />
      ...
      <div className="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="premium-panel premium-hairline rounded-[2rem] p-6 sm:p-8">
          <div className="grid gap-8 lg:grid-cols-[1.35fr_0.7fr_0.85fr]">
            <div>
              <div className="mb-5 flex items-center gap-3">
                <span className="flex h-10 w-10 items-center justify-center rounded-2xl border border-accent-blue/25 bg-accent-blue/10 shadow-[0_0_34px_rgba(77,162,255,0.18)]">
                  <span className="h-3 w-3 rounded-full bg-accent-blue shadow-[0_0_22px_rgba(77,162,255,0.95)]" />
                </span>
                <h2 className="text-xl font-black tracking-[-0.03em] text-text">{brandBlock?.title || 'Planetlocksmiths'}</h2>
              </div>
            ...
            <div>
              ...
              <nav className="mt-4 grid gap-3">
                <Link href={`/${locale}`} className="text-sm transition hover:text-text">{labels.home}</Link>
                <Link href={`/${locale}/services`} className="text-sm transition hover:text-text">{labels.services}</Link>
                <Link href={`/${locale}/areas`} className="text-sm transition hover:text-text">{labels.areas}</Link>
                <Link href={`/${locale}/contact`} className="text-sm transition hover:text-text">{labels.contact}</Link>
                <Link href={`/${locale}/privacy`} className="text-sm transition hover:text-text">{labels.privacy}</Link>
                <Link href={`/${locale}/terms`} className="text-sm transition hover:text-text">{labels.terms}</Link>
                <Link href={navigationBlock?.ctaHref || `/${locale}/contact#request-service`} className="w-fit rounded-full border border-accent-blue/25 bg-accent-blue/10 px-4 py-2 text-xs font-black uppercase tracking-[0.16em] text-accent-blue transition hover:bg-accent-blue/15">{navigationBlock?.ctaLabel || labels.request}</Link>
              </nav>
            </div>
          </div>
