    ...
    <header className="sticky top-0 z-50 border-b border-white/10 bg-bg/95 backdrop-blur-xl">
      <div className="mx-auto flex h-16 max-w-7xl items-center justify-between gap-3 px-4 sm:px-6 lg:px-8">
        ...
        <div className="hidden items-center gap-3 md:flex">
          <Link href={requestHref} className="rounded-full border border-white/15 px-4 py-2 text-sm font-semibold text-text transition-colors hover:border-accent-blue/50 hover:bg-accent-blue/10">
            {labels.request}
          </Link>
        ...
        <div className="border-t border-white/10 bg-bg md:hidden">
          <nav className="mx-auto flex max-w-7xl flex-col px-4 py-4 sm:px-6">
            ...
            <div className="mt-4 grid grid-cols-2 gap-3">
              <Link href={requestHref} className="rounded-full border border-white/15 px-4 py-3 text-center text-sm font-semibold text-text" onClick={() => setOpen(false)}>
                {labels.request}
              </Link>
