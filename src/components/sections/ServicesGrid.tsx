  ...
  return (
    <section className="relative bg-transparent py-16 sm:py-20">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <PremiumReveal className="mb-9 flex flex-col gap-5 md:flex-row md:items-end md:justify-between">
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {services.map((service, index) => (
            <PremiumReveal key={service.slug} delay={Math.min(index * 0.045, 0.18)}>
              <Link href={`/${locale}/services/${service.slug}`} className="group premium-panel relative flex min-h-[17rem] overflow-hidden rounded-[1.5rem] p-6 transition duration-500 hover:-translate-y-1.5 hover:border-accent-blue/40 hover:shadow-[0_0_64px_rgba(77,162,255,0.10)]">
