        ...
        <div className="relative z-10 text-center lg:text-left">
          ...
          <div className="mt-8 flex flex-wrap justify-center gap-2.5 lg:justify-start">
            {(visibleBadges.length ? visibleBadges : ['Car lockouts', 'Keys + fobs', 'Mobile response']).map((badge) => (
              <span key={badge} className="rounded-full border border-white/16 bg-white/[0.06] px-4 py-2 text-xs font-semibold text-text/82 shadow-[inset_0_1px_0_rgba(255,255,255,0.12)] backdrop-blur-2xl">{badge}</span>
            ))}
          </div>
        </div>

        <aside className="premium-panel premium-hairline relative rounded-[2rem] p-5 sm:p-6">
          <div className="relative z-10">
            ...
            <div className="mt-6 grid gap-3">
              {[
                ...
                <div key={step} className="flex items-center gap-3 rounded-2xl border border-white/14 bg-white/[0.055] px-4 py-3 shadow-[inset_0_1px_0_rgba(255,255,255,0.10)] backdrop-blur-2xl">
            ...
            <div className="mt-6 grid grid-cols-2 gap-3">
              <a href={primaryCtaHref} className="inline-flex min-h-11 items-center justify-center rounded-full bg-accent-blue px-4 py-3 text-xs font-black uppercase tracking-[0.14em] text-black transition hover:brightness-110">Call</a>
