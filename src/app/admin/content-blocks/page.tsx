...
const presets: Preset[] = [
  ...
  },
  {
    ...
    body: 'The site is designed around vehicle-specific requests so customers can provide service type, vehicle details, location, urgency, and contact information clearly.',
  },
  {
    label: 'FAQ hero',
    pageKey: 'faq',
  ...
  { label: 'FAQ empty state', pageKey: 'faq', slot: 'empty', body: 'No published FAQ items yet.' },
  {
    label: 'Reviews hero',
    pageKey: 'reviews',
    slot: 'hero',
  ...
  { label: 'Reviews empty state', pageKey: 'reviews', slot: 'empty', body: 'No published reviews yet.' },
  {
    label: 'Contact hero',
    pageKey: 'contact',
    slot: 'hero',
  ...
  { label: 'Contact phone tile', pageKey: 'contact', slot: 'info-phone', title: 'Phone', body: '+1 (215) 555-0100' },
  { label: 'Contact service tile', pageKey: 'contact', slot: 'info-service', title: 'Service type', body: 'Mobile automotive locksmith' },
  { label: 'Contact area tile', pageKey: 'contact', slot: 'info-area', title: 'Common area', body: 'Philadelphia, Pennsylvania and nearby coverage areas' },
  {
  ...
  { label: 'Legal privacy hero', pageKey: 'legal-privacy', slot: 'hero', eyebrow: 'Customer information', title: 'Privacy Policy', body: 'This page explains how Planetlocksmiths handles information submitted through this website for mobile automotive locksmith service requests.' },
  { label: 'Legal privacy section', pageKey: 'legal-privacy', slot: 'section-1', title: 'Information we collect', body: 'When you submit a service request, we may collect your name, phone number, email address, requested service, vehicle make/model/year, service location, urgency, preferred time, and message details.' },
  { label: 'Legal terms hero', pageKey: 'legal-terms', slot: 'hero', eyebrow: 'Customer information', title: 'Terms of Service', body: 'These terms explain the basic conditions for using this website and submitting a mobile automotive locksmith service request to Planetlocksmiths.' },
  { label: 'Legal terms section', pageKey: 'legal-terms', slot: 'section-1', title: 'Website use', body: 'This website provides information about automotive locksmith services and allows customers to submit service requests. You agree to provide accurate contact, vehicle, and location details when requesting service.' },
...
export default function AdminContentBlocksPage() {
  ...
  function addPresetRow() { const preset = presets.find((item) => item.label === selectedPreset) ?? presets[0]; setRowsByLocale((prev) => { const current = prev[activeLocale]; const maxSort = current.length ? Math.max(...current.map((item) => item.sortOrder)) : -1; return { ...prev, [activeLocale]: [...current, createPresetRow(activeLocale, preset, maxSort + 1)] } }); setSuccessMessage(`Preset added: ${preset.label}. Review text and save.`) }
  ...
  const filteredRows = currentRows.filter((row) => { const query = search.trim().toLowerCase(); const matchesSearch = !query || row.pageKey.toLowerCase().includes(query) || row.slot.toLowerCase().includes(query) || row.title.toLowerCase().includes(query) || row.body.toLowerCase().includes(query); const matchesPublish = publishFilter === 'all' ? true : publishFilter === 'published' ? row.isPublished : !row.isPublished; return matchesSearch && matchesPublish })
  ...
  return (
    <div>
      ...
      <div style={guideStyle}>Page keys: <Code>about</Code>, <Code>faq</Code>, <Code>reviews</Code>, <Code>services</Code>, <Code>areas</Code>, <Code>service-detail</Code>, <Code>area-detail</Code>, <Code>contact</Code>, <Code>legal-privacy</Code>, <Code>legal-terms</Code>, <Code>footer</Code>. Detail overrides can use <Code>service:slug</Code> or <Code>area:slug</Code>.</div>
...
function ContentBlockEditor({ row, index, deletingId, onPatch, onDelete }: { row: ContentBlockRow; index: number; deletingId: string | null; onPatch: (patch: Partial<ContentBlockRow>) => void; onDelete: () => void }) {
  return <div style={cardStyle}><div style={cardHeaderStyle}><div><strong style={{ fontSize: 18 }}>Block #{index + 1}</strong><p style={{ margin: '6px 0 0', color: '#95A0B8', fontSize: 13 }}>{row.pageKey} / {row.slot}</p></div><button type="button" onClick={onDelete} disabled={deletingId === row.id} style={dangerGhostButtonStyle}>{deletingId === row.id ? 'Deleting...' : 'Delete'}</button></div><div style={fieldGridStyle}><Field label="Page Key" value={row.pageKey} onChange={(value) => onPatch({ pageKey: value })} /><Field label="Slot" value={row.slot} onChange={(value) => onPatch({ slot: value })} /><Field label="Sort Order" value={String(row.sortOrder)} onChange={(value) => onPatch({ sortOrder: Number(value || 0) })} /></div><Field label="Eyebrow" value={row.eyebrow} onChange={(value) => onPatch({ eyebrow: value })} /><Field label="Title" value={row.title} onChange={(value) => onPatch({ title: value })} /><TextAreaField label="Body" value={row.body} onChange={(value) => onPatch({ body: value })} /><TextAreaField label="Items (one per line)" value={row.itemsText} onChange={(value) => onPatch({ itemsText: value })} /><div style={fieldGridStyle}><Field label="CTA Label" value={row.ctaLabel} onChange={(value) => onPatch({ ctaLabel: value })} /><Field label="CTA Href" value={row.ctaHref} onChange={(value) => onPatch({ ctaHref: value })} /></div><label style={{ display: 'flex', gap: 10, alignItems: 'center' }}><input type="checkbox" checked={row.isPublished} onChange={(event) => onPatch({ isPublished: event.target.checked })} /><span>Published</span></label></div>
}
...
function FilterBar({ search, onSearchChange, searchPlaceholder, filterValue, onFilterChange, filterOptions }: { search: string; onSearchChange: (value: string) => void; searchPlaceholder: string; filterValue: string; onFilterChange: (value: string) => void; filterOptions: Array<{ value: string; label: string }> }) { return <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: 12, marginBottom: 16 }}><input value={search} onChange={(event) => onSearchChange(event.target.value)} placeholder={searchPlaceholder} style={inputStyle} /><select value={filterValue} onChange={(event) => onFilterChange(event.target.value)} style={inputStyle}>{filterOptions.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select></div> }
