...
const fallbackCopy: Record<ActiveLocale, { eyebrow: string; title: string; intro: string; phone: string; serviceType: string; serviceValue: string; area: string; areaValue: string; sideTitle: string; sideText: string; call: string; request: string }> = {
  en: { eyebrow: 'Contact Planet Locksmiths', title: 'Book Mobile Locksmith Service', intro: 'Call or send service details, location, urgency, and contact information. For urgent lockouts, lost keys, business access problems, or safe issues, calling may be faster.', phone: 'Phone', serviceType: 'Service type', serviceValue: 'Mobile locksmith service', area: 'Common area', areaValue: 'Philadelphia, Pennsylvania and nearby coverage areas', sideTitle: 'Details that help us respond', sideText: 'Service type, exact location, phone number, urgency, access authorization, and vehicle details when relevant help confirm the right next step.', call: 'Call', request: 'Request service' },
  es: { eyebrow: 'Contacto Planet Locksmiths', title: 'Reservar servicio móvil de cerrajería', intro: 'Llame o envíe servicio, ubicación, urgencia y contacto. Para bloqueos urgentes, llaves perdidas, acceso comercial o cajas fuertes, llamar puede ser más rápido.', phone: 'Teléfono', serviceType: 'Tipo de servicio', serviceValue: 'Cerrajería móvil', area: 'Área común', areaValue: 'Philadelphia, Pennsylvania y áreas cercanas', sideTitle: 'Detalles que ayudan a responder', sideText: 'Tipo de servicio, ubicación exacta, teléfono, urgencia, autorización y datos del vehículo cuando aplique ayudan a confirmar el próximo paso.', call: 'Llamar', request: 'Solicitar servicio' },
}
      ...
      <main className="flex flex-col">
        <section className="px-4 py-14 sm:px-6 lg:px-8">
          <div className="premium-panel premium-hairline mx-auto grid max-w-7xl gap-8 rounded-[2.25rem] p-6 sm:p-8 lg:grid-cols-[1fr_23rem] lg:items-end lg:p-10">
            <div className="relative z-10"><p className="mb-4 text-xs font-black uppercase tracking-[0.3em] text-accent-cyan">{heroEyebrow}</p><h1 className="max-w-5xl break-keep text-balance text-[2.7rem] font-semibold leading-[0.98] tracking-normal sm:text-6xl sm:leading-[0.94] lg:text-7xl">{heroTitle}</h1><p className="mt-7 max-w-3xl text-base leading-8 text-muted sm:text-lg">{heroIntro}</p><div className="mt-8 flex flex-col gap-3 sm:flex-row"><a href={`tel:${global.phonePrimary}`} className="notranslate inline-flex min-h-12 items-center justify-center rounded-full bg-accent-blue px-7 py-3 text-sm font-black uppercase tracking-[0.16em] text-black shadow-[0_0_44px_rgba(77,162,255,0.32)] transition duration-300 hover:-translate-y-0.5 hover:brightness-110" translate="no" aria-label={`${callLabel} ${global.phoneDisplay}`}>{callLabel}</a><a href={requestHref} className="inline-flex min-h-12 items-center justify-center rounded-full border border-white/18 bg-white/[0.075] px-7 py-3 text-sm font-black uppercase tracking-[0.16em] text-text shadow-[inset_0_1px_0_rgba(255,255,255,0.14)] backdrop-blur-2xl transition duration-300 hover:-translate-y-0.5 hover:border-accent-gold/45 hover:bg-accent-gold/12 hover:text-accent-gold">{requestLabel}</a></div></div>
