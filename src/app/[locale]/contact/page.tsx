...
export default async function ContactPage({ params }: { params: Promise<{ locale: Locale }> }) {
  ...
  const [global, home, blocks] = await Promise.all([
    getGlobalSettingsFromSource(),
    getHomeContentFromSource(activeLocale),
    getContentBlocksFromSource(activeLocale, 'contact'),
  ])
          ...
          <div className="premium-panel premium-hairline mx-auto grid max-w-7xl gap-8 rounded-[2.25rem] p-6 sm:p-8 lg:grid-cols-[1fr_23rem] lg:items-end lg:p-10">
            ...
            <aside className="premium-panel relative z-10 rounded-[1.5rem] p-5">
              <div className="relative z-10">
                ...
                <p className="mt-3 text-sm leading-7 text-muted">{sideBlock?.body || helperBlock?.body || fallback.sideText}</p>
              </div>
            </aside>
          </div>

          <div className="mx-auto mt-6 grid max-w-7xl gap-3 sm:grid-cols-3">
            <InfoTile title={phoneBlock?.title || fallback.phone} value={phoneBlock?.body || global.phoneDisplay} noTranslate />
            <InfoTile title={serviceBlock?.title || fallback.serviceType} value={serviceBlock?.body || fallback.serviceValue} />
            <InfoTile title={areaBlock?.title || fallback.area} value={areaBlock?.body || fallback.areaValue} />
          </div>
        </section>
  ...
  return (
    <div className="premium-panel rounded-2xl p-4">
      <div className="relative z-10">
        ...
        <p className={`mt-2 text-sm text-muted ${noTranslate ? 'notranslate' : ''}`} translate={noTranslate ? 'no' : undefined}>{value}</p>
      </div>
    </div>
  )
}
