...
export default function ServicesPage({ params }: { params: { locale: 'en' | 'es' | 'ru' } }) {
        ...
        <ul className="grid grid-cols-1 sm:grid-cols-2 gap-6">
          {home.featuredServices.map((service) => (
            <li key={service.slug} className="rounded border border-line p-4">
              ...
              <a
                href={`/${params.locale}/services/${service.slug}`}
                className="text-accent-blue underline text-sm"
              >
                Learn more
              </a>
            </li>
