...
const labels: Record<ActiveLocale, { eyebrow: string; overview: string; customerPrep: string; localInfo: string; serviceReady: string; coverageNotes: string; prepItems: string[]; localItems: string[]; defaultServices: string[] }> = {
  en: { eyebrow: 'Local locksmith coverage', overview: 'Area overview', customerPrep: 'What to prepare before service', localInfo: 'Local service information', serviceReady: 'Services commonly requested here', coverageNotes: 'Coverage notes', prepItems: ['Service needed and exact location', 'Lock, door, safe, mailbox, or access-control details', 'Vehicle make, model, and year when relevant', 'Whether keys are lost, broken, locked in, or not programming', 'Phone number for fast confirmation'], localItems: ['Mobile service depends on technician availability and location', 'Response times may vary by traffic, distance, weather, and urgency', 'Final price depends on service details, parts, authorization, and job complexity'], defaultServices: ['Emergency locksmith 24/7', 'Car lockout help', 'Rekey service', 'Commercial locksmith', 'Residential locksmith'] },
  es: { eyebrow: 'Cobertura local de cerrajería', overview: 'Resumen del área', customerPrep: 'Qué preparar antes del servicio', localInfo: 'Información local', serviceReady: 'Servicios comunes aquí', coverageNotes: 'Notas de cobertura', prepItems: ['Servicio requerido y ubicación exacta', 'Datos de cerradura, puerta, caja fuerte, buzón o access control', 'Marca, modelo y año cuando aplique', 'Si las llaves están perdidas, rotas, encerradas o no programan', 'Teléfono para confirmación'], localItems: ['Servicio móvil depende de disponibilidad y ubicación', 'El tiempo varía por tráfico, distancia, clima y urgencia', 'El precio depende de detalles del servicio, piezas, autorización y complejidad'], defaultServices: ['Cerrajero de emergencia 24/7', 'Auto cerrado', 'Rekey', 'Cerrajero comercial', 'Cerrajero residencial'] },
}
...
export async function generateMetadata({ params }: { params: Promise<{ locale: Locale; slug: string }> }): Promise<Metadata> {
  ...
  if (!area) return { title: 'Service Area Not Found | Planet Locksmiths', robots: { index: false, follow: false } }
        ...
        <article className="mx-auto max-w-7xl px-4 py-14 sm:px-6 lg:px-8">
          <section className="premium-panel premium-hairline grid gap-8 rounded-[2.25rem] p-6 sm:p-8 lg:grid-cols-[1fr_23rem] lg:items-end lg:p-10">
            <div className="relative z-10">
              ...
              <p className="mt-7 max-w-3xl text-base leading-8 text-muted sm:text-lg">{heroBlock?.body || area.seoDescription || `Mobile locksmith coverage for ${location}.`}</p>
