      ...
      <main className="relative overflow-hidden px-4 py-16 text-text sm:px-6 lg:px-8">
        <section className="mx-auto max-w-7xl">
          <div className="grid gap-8 rounded-[2rem] border border-white/10 bg-white/[0.045] p-6 shadow-[0_30px_100px_rgba(0,0,0,0.28)] backdrop-blur-xl sm:p-8 lg:grid-cols-[1fr_auto] lg:items-end lg:p-10">
            <div>
              <p className="mb-4 text-xs font-bold uppercase tracking-[0.3em] text-accent-cyan">Planetlocksmiths / coverage</p>
              <h1 className="max-w-4xl text-balance text-4xl font-semibold tracking-[-0.055em] sm:text-5xl lg:text-7xl">Service Areas</h1>
              <p className="mt-6 max-w-3xl text-base leading-8 text-muted sm:text-lg">Mobile automotive locksmith coverage pages help customers understand where service may be available, what support is offered, and what vehicle details are needed before requesting help.</p>
            </div>
          ...
          <div className="mt-10 grid gap-5 md:grid-cols-2 xl:grid-cols-3">
            {areas.map((area, index) => (
              <a key={area.slug} href={`/${locale}/areas/${area.slug}`} className="group relative overflow-hidden rounded-[1.75rem] border border-white/10 bg-white/[0.045] p-6 shadow-[0_24px_80px_rgba(0,0,0,0.24)] backdrop-blur-xl transition hover:border-accent-blue/40 hover:bg-white/[0.06]">
                ...
                <div className="relative">
                  <p className="mb-7 text-[0.65rem] font-bold uppercase tracking-[0.22em] text-accent-gold">Coverage {String(index + 1).padStart(2, '0')}</p>
                  <h2 className="text-balance text-2xl font-semibold tracking-[-0.035em]">{area.title}</h2>
          ...
          {!areas.length ? <div className="mt-8 rounded-[1.5rem] border border-white/10 bg-white/[0.045] p-6 text-muted backdrop-blur-xl">No published areas yet.</div> : null}
        </section>
      </main>
